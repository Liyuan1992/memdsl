...
collapsed: software version `0.9.0` is shipped, while paper version `0.6` is a
working manuscript. The software carries stable/public 0.8 contracts and a
feature-gated workspace-v3 explicit Edge surface that remains experimental.
...
| Software 0.9.0 release | tag `v0.9.0` after the release-scope freeze | Stable/public 0.8 contracts plus feature-gated experimental Phase 6, exact-commit build evidence, and published software artifacts | Paper publication, deployment, authority-runtime conformance, or comparative study |
...
| Bounded Catalog/Query/Trace and explicit v1/v2 serving lanes | Implemented in software 0.9.0 | Phase 2-5 commits and current public API/specification |
...
## Historical 0.6.0 and 0.9.0 release evidence
...
The `0.9.0` software release adds Phase 6 focused tests,
...
python scripts/release_checks.py artifacts --dist dist --expected 0.9.0
python scripts/release_fresh_install.py --wheel dist/memdsl-0.9.0-py3-none-any.whl --workspace examples/alex --expected 0.9.0
```
...
These 0.8.0 archives remain local historical evidence only. The `0.9.0`
archive hashes, member counts, and fixed epoch are recorded after
...
- **Remote actions:** this metadata record does not authorize or imply a DOI,
  Zenodo record, paper submission, host deployment, or private-data activation.
  Software tag, GitHub Release, and PyPI state must be verified from their
  external records.
