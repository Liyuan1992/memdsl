...
- 状态：架构提案，分阶段实施中；Phase -1、0A、0B、1、2、3 已落地
- 当前基线：memdsl 0.6.0，Phase 3 本提交（parent `c13d1e392bfb459f436f2075a7c0b8a25c3a9063`）
...
8. logical revision family 是否需要 schema `family_key_fields`？
9. 一等 edge 最终采用 core syntax 还是 domain declaration？
10. 语义历史是否永久留在热 Source，何时需要 cold-history contract？
11. 新能力的目标版本和 deprecation window 多长？
12. raw `.mem` 文件混合不同 access policy 时，是否拒绝整文件读取、提供授权 source projection，还是要求物理文件分区？
13. hard constraint 正文不可读但可 enforce 时，opaque verdict 的公共 envelope 如何定义？
14. v1 多 module 文件的 report diagnostic 严重级别、deprecation window 和自动迁移建议是什么？
15. parse/schema 已损坏时，repair lane 只允许直接 source edit，还是需要独立的 governed source-patch proposal？
