...
`superseded_ids()` 收集每条声明 relations 中所有 `supersedes` target。Map/query 再把这些 target 排除。
...
还有一个必须先修的 authority 边界缺陷：`superseded_ids()` 当前从**所有** declaration 收集 `supersedes`，不检查发起声明的 lifecycle status、runtime authority、结构有效性或可见性。

这意味着一条 candidate、retracted 或 archived declaration 也可能隐藏 active target。用当前 0.6.0 源码构造：
...
不同读取面目前也没有共享一个统一 current view。Map/query 会额外排除 relationally superseded target；`memory_list`、status counts 和 `workspace_vocabulary` 主要使用 `ws.active()`，可能仍列出或统计同一旧 target。因此同一 workspace 在不同工具中会出现不同的“当前数量”和“当前声明”。

当前 `unmarked_supersede_status` warning 还与 append-only revision 合同存在张力：warning 建议把旧节点原地改成 `status: superseded`；而规范的审核修订路径要求追加新 declaration + supersedes，不静默改写已批准历史。ResolvedView 应由 incoming authoritative supersedes edge 决定 View exclusion，并重新定义或弃用这条 warning，不能要求用户通过改写历史节点消除它。

### 3.4 Reference resolution 的当前不一致

当前 full id 和 bare name 没有统一 resolver：

- `by_id("shared")` 返回 declaration list 中第一个 bare-name 命中；
- 不同 kind 可以共享 bare name；
- lint 把 `target.split(":", 1)[-1]` 作为 bare fallback；
- runtime supersedes/explain incoming checks 又主要做 exact id/name 对比。
...
18. `unmarked_supersede_status` 如何迁移，避免要求改写 append-only 历史？
19. v1 多 module 文件的 report diagnostic 严重级别、deprecation window 和自动迁移建议是什么？
20. parse/schema 已损坏时，repair lane 只允许直接 source edit，还是需要独立的 governed source-patch proposal？
...
| MCP list/status 与 Map current-set 不一致来源 | [mcp_service.py](../src/memdsl/mcp_service.py#L317)、[mcp_service.py](../src/memdsl/mcp_service.py#L641) |
| MCP raw files/path/content | [mcp_service.py](../src/memdsl/mcp_service.py#L932) |
| compliance 无条件使用 superseded_ids | [compliance.py](../src/memdsl/compliance.py#L170) |
| lint relation target 和 unmarked status | [linter.py](../src/memdsl/linter.py#L154)、[linter.py](../src/memdsl/linter.py#L289) |
...
| candidate/retracted/archived supersedes active | active target 被 Map/query 排除 | strict xfail，共 3 实例 | Phase 0A |
| candidate supersedes global constraint | compliance `block -> allow` | strict xfail | Phase 0A |
| Map/list/status/vocabulary current-set | 2 / 3 / 3 / 3 不一致 | strict xfail | Phase 0A |
...
| ambiguous bare ref | 同 bare name 的两个 target 都被排除 | 合并 strict xfail（不冻结 resolver envelope） | Phase 0B/1 |
| wrong kind prefix | lint 通过，runtime 不链接 | 同上 | Phase 0B/1 |
| unknown relation typo | nested key 被静默丢弃 | 同上 | Phase 0B/1 |
| duplicate full id | lint error；Map 两 occurrence；explain 取第一条 | strict xfail（只要求不返回普通 ok） | Phase 0B/1 |
