...
- 状态：Phase -1、0A、0B、1、2、3、4、5 已落地；Phase 6/7 Entry-Gate 已审计并延期
- 当前基线：Phase 5 commit `8e7c84815897ad13f89522e9e3b1edd0fcdb37b0`（parent `e128d351b670bfb1aabe04fa36f5bcdaa4d0b1cb`）；包版本元数据仍为已发布的 0.6.0
- 目标版本：Phase -1 至 Phase 5 发布候选冻结为 0.8.0；Phase 6/7 不进入该 release
...
静态审计当前 workflow：core job 覆盖 Python 3.9-3.12、Windows/Linux、pytest、
CLI smoke 和 build/twine；MCP job 在 Windows/Linux Python 3.12 安装 MCP extra、跑
全套 pytest 和 `--inspect`。由于 Phase 3/4/5 测试中已有真实 stdio client 测试，
该 job 在当前 source line 上会覆盖 stdio，但远端尚无包含这些提交的 CI run，不能把
workflow 形状当作远端已通过证据。

0.8.0 发布候选仍需补齐：

- MCP Python 3.10/3.12 代表矩阵；
- 在远端 CI 上实际跑过当前阶段链的 stdio、scope denial、v1/v2/quarantine tests；
- wheel/sdist member listing 和私密路径/Source/cache 扫描；
- outside-repo fresh wheel install；
- 将 synthetic scale、v1/v2 differential、access/quarantine/cursor/security
  stop-condition 作为明确 CI gate，而不只依赖开发者本地收据；
- publish workflow 安装 MCP extra，避免 `pytest` 因 optional dependency 缺失而跳过
  stdio tests，并在 publish 前执行 twine/member/privacy/fresh-install smoke。
...
- 精确 source 起点为 `8e7c84815897ad13f89522e9e3b1edd0fcdb37b0`，后续发布候选
  整合只能增加版本、文档、CI、迁移、回滚和 release evidence 变更；
...
| 区域 | 当前事实 | 发布前必须完成 |
| --- | --- | --- |
| 版本 | `pyproject.toml`、`src/memdsl/__init__.py`、tests 和 CI wheel assertion 仍硬编码 0.6.0 | 一致 bump 到 0.8.0，并验证 tag/version/package metadata 一致 |
| 文档 | SPEC 仍标 0.6；CHANGELOG 没有 Phase 0A-5；README/PUBLIC_API/UPGRADING 仍称 source-line、scope 未冻结 | 写 0.8.0 release notes，统一公开/experimental 边界、迁移、降级和兼容窗口 |
| Python/MCP matrix | 本机只有 Python 3.12；workflow core 有 3.9-3.12，MCP 只有 3.12 | 远端验证 core 3.9-3.12，并增加 MCP 3.10/3.12 代表矩阵 |
| CI | 当前阶段链没有远端 run；artifact/privacy/fresh-install 仍主要是本地证据 | 远端跑全套 stdio/security/scale gates，列出 artifacts 并做 privacy scan，仓库外 wheel smoke |
| publish workflow | 只安装 `.[dev]`，MCP tests 可 skip；无 twine/member/privacy/fresh-install gate | 在 tag publish 前安装 MCP extra并执行完整 release gate |
| 包证据 | Phase 5 本地记录 wheel 26 / sdist 94 members；本窗口隔离环境复核 `351 passed`、21 个 core Python 3.9 AST parse 和 `pip check` | 在最终 0.8.0 clean candidate 上重新 build、twine、member/privacy、fresh wheel + MCP smoke；旧 0.6.0 member count 不能复用 |
| 环境隔离 | 共享原始 checkout 的 editable venv 会让 stdio 子进程导入错误 source；独立 temp venv 安装当前 worktree 后为 351/351 | 发布候选必须从 clean isolated venv/wheel 运行，记录 import path，不能依赖其他 checkout 的 editable install |
| 远端状态 | `origin/main` 与 `v0.6.0` 均为 `72274d9`；Phase 0A-5 和本审计均未 push；PyPI 0.6.0 存在但 GitHub 没有 v0.6.0 release object | 仅在用户明确授权后 push candidate、等待 CI、tag、创建 release 并验证 PyPI；默认停止在本地证据 |

只有上述硬缺口关闭，0.8.0 才能称为 release candidate ready；本节本身不是发布完成证明。
