...
- 状态：架构提案，分阶段实施中；Phase -1、0A、0B、1、2 已落地
...
| D-028 | Phase 1 duplicate identity 采用 report + single-resolution fail-loud：集合保留 occurrences，explain 不选首项 | report-only 不能擅自整库隔离，但单值解释不能继续由文件顺序偶然决定 | Map/query 全部立即拒服，或继续 first occurrence serving | 接受 Phase 1 行为；Phase 5 blocking 范围仍开放 |
...
Phase 2 已关闭原第 8-10 项：Catalog 采用独立 new tool/schema；cursor 为无状态
opaque token；默认预算为 20 items / 8192 canonical compact UTF-8 JSON bytes。
以下问题仍需在进入对应实现阶段前决策：

1. `use X` 精确导入 symbol、module 还是 namespace？
2. built-in relation descriptor 在 Phase 1 先使用内部 core registry；未来 custom relation descriptor 属于 schema 还是 manifest？
3. `supersedes`/`revision_of` 已冻结为 acyclic；未来 custom relation 如何声明 acyclic？
4. revision fork 的 report mode 已冻结为“不选 winner、两个有效 successor 保持可见并报告”；quarantine/strict 中的精确污染范围是什么？
5. duplicate full id 在 Phase 1 report mode 保留集合 occurrences、拒绝单值 explain；quarantine/strict 中是否永远 workspace-blocking？
6. access-policy filtering 的公共 Python API 形状？
7. `view_id` 是否包含 principal digest，如何避免侧信道？
8. vocabulary suggestion 是否独立工具？本文倾向先合入 query。
9. logical revision family 是否需要 schema `family_key_fields`？
10. 一等 edge 最终采用 core syntax 还是 domain declaration？
11. 语义历史是否永久留在热 Source，何时需要 cold-history contract？
12. 新能力的目标版本和 deprecation window 多长？
13. raw `.mem` 文件混合不同 access policy 时，是否拒绝整文件读取、提供授权 source projection，还是要求物理文件分区？
14. hard constraint 正文不可读但可 enforce 时，opaque verdict 的公共 envelope 如何定义？
15. v1 多 module 文件的 report diagnostic 严重级别、deprecation window 和自动迁移建议是什么？
16. parse/schema 已损坏时，repair lane 只允许直接 source edit，还是需要独立的 governed source-patch proposal？
