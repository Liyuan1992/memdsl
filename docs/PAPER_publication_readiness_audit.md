...
- **Software release:** unchanged. `0.8.0` remains an unreleased local
  candidate. No P5 evidence is a remote CI run, tag, GitHub Release, Zenodo
  record, DOI, PyPI release, production rollout, or empirical result.
...
| Deferred practical phases | `ff9ff8a7dfe983a9db3caaf221d7315cfda8eba6` | Phase 6 first-class edges and Phase 7 cold-history/incremental compilation were audited and deferred, not implemented |
| Unreleased local candidate | `091b4736e02969ccdd198cfcb30ba9038feb78db` | Local `0.8.0` integration and release-gate evidence only |
...
shipped `0.6.0`, implemented Phase -1 through 5, deferred Phase 6/7, the local
`0.8.0` candidate, and the unimplemented paper contract.
...
| Implementation status | Accurate but nonconforming | Shipped `0.6.0`, implemented Phase -1 through 5, deferred Phase 6/7, local `0.8.0` candidate, and planned paper runtime remain distinct. |
...
| Privacy | Ready for the audited local sources | Paper artifacts contain synthetic/public material and no private memory workspace, proposal store, audit log, credential, or personal dataset. Artifact and machine-path scans remain required in P6. |
...
6. The `0.8.0` candidate remains local; remote CI, tag, GitHub Release, Zenodo,
   DOI, and PyPI publication have not occurred.
