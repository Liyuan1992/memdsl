...
### 0.9.0.dev0 experimental line

This branch adds an opt-in `memdsl.workspace.v3` first-class explicit Edge
experiment. It does not rewrite the frozen 0.8 candidate: legacy relations and
...
This repository state is an unreleased local `0.9.0.dev0` candidate. The
shipped software remains `0.6.0`; no remote CI result, tag, GitHub Release, or
PyPI publication is implied by the candidate documentation.
...
- ~~Public rebuildable CompiledWorkspace + bounded Catalog pagination/budgets~~ — stable/public 0.8 contract carried by the local 0.9 dev candidate
- ~~Indexed query + bounded Trace + exact use/report/strict + reviewed Dialect~~ — stable/public 0.8 contract carried by the local 0.9 dev candidate
...
- ~~Opt-in first-class reviewed Edge experiment~~ — added in `0.9.0.dev0`
...
本分支新增显式 opt-in 的 `memdsl.workspace.v3` 一等 Edge 实验，不改写已经冻结的
0.8 candidate：legacy relation 与 authority 保持兼容，Edge proposal 永远进入人工
...
0.7.0。正式公共合同包括 Source/review authority、公共且可重建的
...
当前仓库状态是未发布的本地 `0.9.0.dev0` candidate。已发布软件仍为 `0.6.0`；
候选文档不代表远端 CI、tag、GitHub Release 或 PyPI 发布已经发生。
...
- ~~公共可重建 CompiledWorkspace + 有界 Catalog 分页/预算~~ — 当前 0.9 dev 候选承载的 stable/public 0.8 合同
- ~~倒排查询 + 有界 Trace + exact use/report/strict + 受审核 Dialect~~ — 当前 0.9 dev 候选承载的 stable/public 0.8 合同
- ~~显式 v2 envelope 下的 opt-in quarantine enforcement~~ — schema/安全合同 stable/public，真实 rollout 质量仍为 experimental
- ~~显式 opt-in 的一等可审核 Edge 实验~~ — `0.9.0.dev0` 已基于匿名探索性
