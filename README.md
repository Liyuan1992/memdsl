...
- ~~Internal compiled indexes + bounded Catalog pagination/budgets~~ — shipped in v0.8
- ~~Indexed query + bounded Trace + exact use/report/strict + reviewed Dialect~~ — shipped in v0.8
- ~~Opt-in quarantine enforcement with explicit v2 envelopes~~ — shipped in v0.8; rollout remains experimental
...
- ~~内部编译索引 + 有界 Catalog 分页/预算~~ — v0.8 已发布范围
- ~~倒排查询 + 有界 Trace + exact use/report/strict + 受审核 Dialect~~ — v0.8 已发布范围
- ~~显式 v2 envelope 下的 opt-in quarantine enforcement~~ — v0.8 已发布范围，真实 rollout 仍为 experimental
