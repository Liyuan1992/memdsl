...
The software release is `0.9.0`, anchored by tag `v0.9.0`. Package-level
release does not promote the opt-in Edge surface to stable, publish the
companion paper, or claim that the paper authority runtime is implemented.
...
正式软件版本为 `0.9.0`，由 tag `v0.9.0` 锚定。软件版本发布不会把 opt-in Edge
提升为 stable，也不代表配套论文已经发表，更不代表论文 authority runtime 已实现。
