...
retry signal, and preflight consequential drafts against MUST constraints with
...
that misses just returns nothing. Three surfaces support that loop:
...
- **Map v1 compatibility** (`memdsl map`, MCP `memory_map` / `memdsl://map`,
  ...
  session-start surface. Map and Catalog are navigation, not citation.
...
- tools: `memory_catalog`, `memory_map`, `memory_types`, `memory_query`, `memory_check`,
...
bounded as the workspace grows. `memory_map` remains for v1 clients. Before
...
继续 cursor，把 `no_match` 当作重试信号，并在产出重要草稿前用
...
- **Map v1 兼容面**（`memdsl map`、MCP `memory_map` / `memdsl://map`、Python
  ...
  大型 workspace 的会话起始 surface。Map 与 Catalog 都只是导航，不是引用来源。
...
线性膨胀；`memory_map` 继续服务 v1 客户端。在提出新声明前，应该先调用
