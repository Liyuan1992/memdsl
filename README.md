...
author's ontology on every user**. Version 0.6 keeps the two-layer type
architecture introduced in 0.5 and adds lifecycle-safe provisional serving
plus risk-tiered review:
...
is the 0.6-line compatibility authority rule. The later Phase 5 ResolvedView
uses the same fail-safe base and adds only explicit v2 quarantine enforcement.
...
Workspace v2 can independently enable a Phase 5 read gate:
...
JSON output remains `memdsl.evidence_pack.v1`; version 0.6 additively adds
`provisional`. Scored CONTEXT and PROVISIONAL entries include `score` and
...
- **Map v1 compatibility** (`memdsl map`, MCP `memory_map` / `memdsl://map`,
  ...
  session-start surface. Under explicit Phase 5 enforcement it returns
...
in [docs/DESIGN_review_policy.md](docs/DESIGN_review_policy.md).
...
that one ontology generalizes to everyone. The v0.6 architecture makes that
...
- ~~Internal compiled indexes + bounded Catalog pagination/budgets~~ — Phase 0B/2 source line
- ~~Indexed query + bounded Trace + exact use/report/strict + reviewed Dialect~~ — Phase 3/4 source line
- ~~Opt-in quarantine enforcement with explicit v2 envelopes~~ — Phase 5 source line
...
v0.6 保留 v0.5 引入的两层类型架构，并加入 lifecycle 安全的 provisional
服务和分级审核：
...
compliance。这是 0.6 线的 correctness/security 兼容 authority 规则。后续
Phase 5 ResolvedView 复用这个安全基础，只在显式 v2 opt-in 时增加 quarantine
enforcement。
...
workspace v2 可以独立开启 Phase 5 读取 gate：
...
JSON 输出仍为 `memdsl.evidence_pack.v1`；v0.6 以 additive 方式新增
`provisional`。带分数的 CONTEXT 和 PROVISIONAL 条目都包含 `score`、
...
- **Map v1 兼容面**（`memdsl map`、MCP `memory_map` / `memdsl://map`、Python
  ...
  大型 workspace 的会话起始 surface。显式 Phase 5 enforcement 下会返回
...
ontology 对所有人都通用。v0.6 正面承认这个限制：memdsl 标准化记录结构和运行
契约，把领域词汇的所有权交还给使用者。
...
- ~~内部编译索引 + 有界 Catalog 分页/预算~~ — Phase 0B/2 source line 已完成
- ~~倒排查询 + 有界 Trace + exact use/report/strict + 受审核 Dialect~~ — Phase 3/4 source line 已完成
- ~~显式 v2 envelope 下的 opt-in quarantine enforcement~~ — Phase 5 source line 已完成
